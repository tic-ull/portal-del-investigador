...
USERNAME = 'testCVN'
PASSWORD = '123'
ATTR = {'username': 'testCVN', 'first_name': 'Test',
        'last_name': 'Testing CVN', 'TipoDocumento': 'NIF',
        'NumDocumento': '123456789V',
        'ou': ['PDI', 'srv_wifi', 'Alumnos', 'srv_soft', 'rol_becario',
               'srv_webpages', 'srv_ddv', 'srv_vpn', 'CCTI', 'srv_siga_ull.es',
               'NACFicheros', 'Act', 'srv_google', 'srv_siga'],
        'email': 'test@ull.edu.es'}
TEST_LOGIN = 'http://testserver/investigacion/accounts/login/'
TEST_LOGIN_URL = TEST_LOGIN + '?next=/investigacion/cvn/download/'
# TODO: Añadir la ruta desde una variable del settings
# que ya está configurada para ello.
...
class UserAccessTest(TestCase):
    ...
    def setUp(self):
        """
            Método que se ejecuta antes de cada test de la clase para la
            configuración inicial de los mismos
        """
    ...
    def test_download_cvn_anonymous(self):
        """
            Test: Usuario anónimo accede a la ruta (de forma manual)
                de descarga del CVN -> redirección a la página de acceso CAS.
        ...
        # Se redirige hacia la página de acceso CAS
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response._headers['location'][1], TEST_LOGIN_URL)

    def test_download_cvn_user_withoutCVN(self):
        """
            Test: Usuario registrado que no ha subido CVN, accede a la
            página de descarga (añadiendo url de forma manual) -> error 404.

            Notas:
                404 Not Found: The server has not found anything matching
                the Request-URI.

            Importante: Se lanza el 404 en el código con un "raise http404".
             Cambiar si es necesario por un error más aproiado.
        """
        ...
        try:
            ...
            # Caso de que quede el registro almacenado en la BBDD de test
            pass
        # Descarga del CVN
    ...
    def test_download_cvn_user_withCVN(self):
        """
            Test: Usuario registrado con CVN subido,
            accede a la descarga del mismo.
        ...
        GrupoinvestInvestcvn.objects.create(pk=ID_TEST, investigador=invest,
                                            cvnfile=CVN_FILE)
    ...
    def test_login_redirect(self):
        """
            Test: Usuario anónimo accede a la aplicación -> redirección
                hacia la página del acceso CAS.

            Notas:
                302 Found: The requested resource resides temporarily under a
                different URI.
        """
        # Accede un usuario anónimo,
        # tiene que ser redirigido a la página de acceso
...
class FecytWSTest(TestCase):
    ...
    def setUp(self):
        """
            Método que se ejecuta antes de cada test de la clase
            para la configuración inicial de los mismos
        """
        self.clientWS = suds.client.Client(URL_WS)

    def __checkDataPublicacion__(self, data=None, libro=False):
        """
            Función auxiliar que comprueba que los datos de las
            publicación han sido introducidos en la BBDD

            Variables:
                * data: Registro de tipo publicación con todos los
                datos que debieron ser introducidos.
                * libro: Indica que la publicación se trata de un libro,
                 con lo que no se mira el ISSN
        """
        if not data.titulo:
            return False
    ...
    def __checkDataExpCientifica__(self, data=None, proyecto=False):
        """
            Función auxiliar que se encarga de comprobar que los datos
            de los Proyectos y Convenios son insertados correctamente.

            Variables:
            * data: Registro que contiene todos los datos que debieron
                 ser introducidos.
            * proyecto: Indica si se trata de un proyecto (True)
                o convenio (False)
        """
    ...
    def __checkDataTesis__(self, data=None):
        """
            Función auxiliar que comprueba que los datos de una Tesis
            se han introducido correctamente en la BBDD.

            Variable:
            * data: Registro de tipo tesis que contiene todos los datos
                que debieron ser introducidos.
        """
        if not data.titulo:
            return False
    ...
    def __checkDataCongreso__(self, data=None):
        """
            Función auxiliar que comprueba que los datos de un congreso
            se han introducido correctamente en la BBDD.
        """
        if not data.titulo:
            return False
    ...
    def __checkPublicacion__(self, user=None):
        """
            Función auxiliar que se encarga de comprobar que se han
             añadido los artículos correctos.

            Variables:
            * user: Usuario 'Test' propietario de la actividad científica.
        """
        pub = user.publicacion_set
        self.assertEqual(pub.filter(tipo_de_produccion__icontains="Capítulo")
                            .count(), 3)
        self.assertEqual(pub.filter(tipo_de_produccion__iexact="Artículo")
                            .count(), 2)
        self.assertEqual(pub.filter(tipo_de_produccion__iexact="Libro")
                            .count(), 1)
        # Se comprueba que los datos del Artículo se han introducido en la BBDD
        articulo = pub.get(titulo__icontains="Artículo 1")
        self.assertTrue(self.__checkDataPublicacion__(articulo))
        capitulo = pub.get(titulo__icontains="Capítulo 1")
        self.assertTrue(self.__checkDataPublicacion__(capitulo))
        libro = pub.get(titulo__icontains="Libro 1")
    ...
    def test_insertCVN(self):
        ...
        self.assertEqual(user.publicacion_set.count(), 6)
        # Número de artículos, capítulos y libros por separado.
        # Se comprueba también que se introducen sus datos correctamente.
        self.assertTrue(self.__checkPublicacion__(user))

        # Número de Proyectos y Convenios
        self.assertEqual(user.proyecto_set.count(), 2)
        proyecto = user.proyecto_set.get(
            denominacion_del_proyecto="Proyecto 1")
        self.assertTrue(self.__checkDataExpCientifica__(proyecto, True))
        self.assertEqual(user.convenio_set.count(), 3)
        convenio = user.convenio_set.get(
            denominacion_del_proyecto="Convenio 1")
        self.assertTrue(self.__checkDataExpCientifica__(convenio))

        # Número de Tesis
        self.assertEqual(user.tesisdoctoral_set.count(), 2)
        tesis = user.tesisdoctoral_set.get(titulo="Tesis Doctoral 1")
        self.assertTrue(self.__checkDataTesis__(tesis))

        # Número de congresos
        self.assertEqual(user.congreso_set.count(), 3)
        congreso = user.congreso_set.get(titulo="Congreso 1")
        self.assertTrue(self.__checkDataCongreso__(congreso))
