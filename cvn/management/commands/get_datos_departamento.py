# -*- encoding: utf8 -*-
from date_string_helpers import utf8

class Get_datos_departamento:
    """
    clase que consulta una BBDD a través de una conexión y obtiene todos los datos
    referentes a un departamento (instituto):
    Entrada: 
        Conexión a la BBDD
    ...
    Salida:
        ...
        investigadores: lista de tuplas con (nombre, primer apellido, segundo apellido, categoria)
    ...
    def __init__(self, db_connection, identificador, year, tipo="departamento"):
    ...
    def get_datos_basicos(self):
        ...
        instruccion = """SELECT nombre 
                         FROM {0} 
                         WHERE id = {1}""".format(self.table, self.identificador)           
    ...
    def query_data_investigadores(self):
        ...
        instruccion = """SELECT DISTINCT nombre, apellido1, apellido2, nif 
                         ...
                         WHERE {1} = {2} 
                         ORDER BY apellido1""".format("mem12", self.tipo_id, self.identificador)
        cursor = self.db_connection.cursor()
        cursor.execute(instruccion)
    ...
    def set_inst_base(self):
        self.inst_base = """
                            ...
                            WHERE documento IN (
                                SELECT DISTINCT nif 
                                FROM {0}_GrupoInvest_investigador 
                                WHERE {1} = {2})""".format("mem12", self.tipo_id, self.identificador)
        

    def dept_publicacion(self, tipo, instruccion_base):
        instruccion = u"""
                        SELECT DISTINCT fecha, titulo, nombre_publicacion, autores, issn, volumen, numero, pagina_inicial, pagina_final
                        FROM {0}_cvn_publicacion 
                        WHERE YEAR(fecha) = {1} AND 
                              tipo_de_produccion LIKE '{2}' AND 
                              id IN ({3})
                        ORDER BY fecha""".format("mem12", self.year, tipo, instruccion_base)
        return instruccion          

    def get_produccion(self):
        """
        para cada tipo de produccion realiza la consulta y rellena el diccionario
        """
        inst_publicacion = u"""
                            SELECT DISTINCT publicacion_id 
                            FROM {0}_cvn_publicacion_usuario 
                            WHERE usuario_id IN ({1})""".format("mem12", self.inst_base)
 
        inst_tipo_public = u"""
                            SELECT id 
                            FROM {0}_cvn_publicacion 
                            WHERE id IN ({1})""".format("mem12", inst_publicacion)
    ...
    def dataCongresos(self): 
        ...
        instruccion = """
                         SELECT DISTINCT titulo, nombre_del_congreso, autores, ciudad_de_realizacion, fecha_realizacion
                         FROM {0}_cvn_congreso 
                         WHERE YEAR(fecha_realizacion) = {1} 
                         AND id IN (
                         {2}) 
                         ORDER BY fecha_realizacion""".format("mem12", self.year, inst_congreso)
        cursor = self.db_connection.cursor()
        cursor.execute(instruccion)
    ...
    def dataProyectos(self):
        inst_proyecto = """
                           ...
                           WHERE usuario_id IN (
                               {1})""".format("mem12", self.inst_base)
        instruccion = """
                         SELECT DISTINCT denominacion_del_proyecto, fecha_de_inicio, fecha_de_fin, cuantia_total, autores 
                         FROM {0}_cvn_proyecto 
                         WHERE (YEAR(fecha_de_inicio) = {1} OR (YEAR(fecha_de_inicio)<{1} AND YEAR(fecha_de_fin)>= {1}))
                         AND id IN (
                         {2}) 
                         ORDER BY fecha_de_inicio""".format("mem12", self.year, inst_proyecto);
        cursor = self.db_connection.cursor()
        cursor.execute(instruccion)
    ...
    def dataConvenios(self): 
        inst_convenio = """
                        ...
                        WHERE usuario_id IN (
                            {1})""".format("mem12", self.inst_base)
        instruccion = """
                    SELECT DISTINCT denominacion_del_proyecto, fecha_de_inicio, cuantia_total, autores, duracion_anyos, duracion_meses, duracion_dias 
                    FROM {0}_cvn_convenio 
                    WHERE ((YEAR(fecha_de_inicio) = {1}) OR (duracion_anyos < 100 AND YEAR(INTERVAL (duracion_anyos * 365 + duracion_meses * 12 + duracion_dias) DAY + fecha_de_inicio) >= {1})
                    OR 
                    (duracion_anyos >= 100 AND YEAR(INTERVAL (duracion_meses * 12 + duracion_dias) DAY + fecha_de_inicio) >= {1})
                    OR 
                    (duracion_anyos >= {1})) AND id IN (
                    {2})
                    ORDER BY fecha_de_inicio""".format("mem12", self.year, inst_convenio);
        cursor = self.db_connection.cursor()
        cursor.execute(instruccion)
    ...
    def dataTesis(self):
        ...
        inst_tesis = """
                     ...
                     WHERE usuario_id IN (
                        {1})""".format("mem12", self.inst_base)
        instruccion = """
                      SELECT DISTINCT id, titulo, fecha_de_lectura, autor, universidad_que_titula, codirector 
                      FROM {0}_cvn_tesisdoctoral 
                      WHERE YEAR(fecha_de_lectura) = "{1}" AND 
                            id IN (
                            {2}) 
                      ORDER BY fecha_de_lectura""".format("mem12", self.year, inst_tesis)
