...
from date_string_helpers import getMonthText, cambia_fecha_a_normal, calcular_duracion, utf8
...
class Informe_pdf:
    ...
    def __init__(self, year, departamento, investigadores, produccion, actividad):
        self.year = year
    ...
    def myFirstPage(self, canvas, doc):
        ...
        canvas.drawString(self.MARGIN, self.PAGE_HEIGHT - 2 * self.MARGIN, self.title)
    ...
    def myLaterPages(self, canvas, doc):
        ...
        canvas.drawCentredString(self.PAGE_WIDTH / 2.0, 
                                ...
                                u"pág. {0} - {1}".format(doc.page, self.title))                                                                                    
        canvas.restoreState()
    ...
    def datos_departamento(self):
        """ 
        a este método se le pasarán los datos del departamento cuando sea el momento
        Devuelve un párrafo (Paragraph)
        """    
        texto = """<b>Dirección:</b> {0}<br/>
<b>Teléfono:</b> {1}<br/>
<b>Fax:</b> {2}<br/>
<b>Correo electrónico:</b> {3}</br>""".format("C/S. Foo de Bar", "6666666666", "922922922", "foobar@ull.es")
    ...
    def tabla_investigadores(self):
        ...
        LIST_STYLE = TableStyle([('SIZE', (0,0), (-1,-1), 8), # tamaño de la letra
                                 ('INNERGRID', (0,0), (-1,-1), 0.25, colors.gray), # grid
                                 ('BOX', (0,0), (-1,-1), 0.25, colors.black), # caja externa

                                 ('ALIGNMENT', (0,0), (-1,0), 'CENTER'), 
                                 ('BACKGROUND', (0,0), (-1,0), colors.black), # fondo cabecera
                                 ('TEXTCOLOR', (0,0), (-1,0), colors.white), # texto cabecera
                                 ('ROWBACKGROUNDS', (0,1), (-1,-1),  [colors.white, colors.lightgrey]), # fondo de filas alternas
                                 ])
    ...
    def lista_produccion(self):
        ...
        for tipo_publicacion, publicaciones in self.produccion.items():
            ...
            if num_publicaciones > 0:
                paragraphs.append(self.lista_publicaciones(tipo_publicacion, 
                                                    num_publicaciones,
                                                    publicaciones))
        return paragraphs
    
    def lista_publicaciones(self, tipo_publicacion, num_publicaciones, publicaciones): 
        texto = "<b>{2}{0}</font> [{1}]</b><br/>".format(utf8(tipo_publicacion),
                                                                     num_publicaciones, 
                                                                     self.SUBTITLE_STYLE) 
    ...
    def lista_convenios(self):
        ...
        if num_convenios > 0:
            ...
            for convenio in convenios:
                ...
                denominacion_del_convenio, fecha_de_inicio, cuantia_total, autores, duracion_anyos, duracion_meses, duracion_dias = convenio
                # TODO revisar este orden
                if denominacion_del_convenio:
                    texto += "Título: {0}.<br/>".format(utf8(denominacion_del_convenio))
                if fecha_de_inicio:
                    new_fecha_de_inicio = cambia_fecha_a_normal(fecha_de_inicio)
                    texto += "Fecha de inicio: {0}<br/>".format(utf8(new_fecha_de_inicio))
                    
                    fecha_de_fin = calcular_duracion(fecha_de_inicio, duracion_anyos, duracion_meses, duracion_dias);
    ...
    def lista_tesis(self):
        ...
        texto = ""
        if num_tesis > 0:
            texto = "<b>{1}Tesis doctorales</font> [{0}]</b><br/>".format(num_tesis, self.SUBTITLE_STYLE)
            for t in tesis:
                (id,
                 titulo, 
                 fecha_de_lectura, 
                ...
                if universidad_que_titula:
                    texto += "Universidad que titula: {0}<br/>".format(utf8(universidad_que_titula))
                        
                if autor:
                    texto += "Título: {0}.<br/>".format(utf8(autor))
                
                if dir_apellido1:
                    texto += "Director:  {0} {1} {2}<br/>".format(utf8(dir_nombre), 
                                                                  utf8(dir_apellido1), 
                                                                  utf8(dir_apellido2))
                ...
                if fecha_de_lectura:
                    ...
                    texto += "Fecha de lectura:  {0}<br/>".format(utf8(fecha_de_lectura))
                texto += "<br/>"
            # end for tesis
        p = Paragraph(texto, self.SECTION_STYLE)
        return p      
