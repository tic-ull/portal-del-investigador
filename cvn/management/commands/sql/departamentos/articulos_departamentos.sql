/*----------------------------------------------------------------------------------------------
-- Artículos únicos por departamento en tabla Publicación-Usuario
----------------------------------------------------------------------------------------------*/
SELECT * FROM
(
SELECT nombre AS "Departamento", 
		COUNT(nombre) AS "Artículos publicados en 2012" 
FROM (
	SELECT DISTINCT mem12_cvn_publicacion_usuario.publicacion_id, 
		...
		FROM mem12_cvn_publicacion
			...
			WHERE YEAR(fecha)=2012 AND 
						tipo_de_produccion="Articulo" AND
...
/*INTO OUTFILE "/tmp/articulos_departamentos_2012.csv" FIELDS TERMINATED BY '|' LINES TERMINATED BY '\n';*/
