...
SELECT nombre AS "Departamento", 
		COUNT(nombre) AS "Capítulos de libro publicados en 2012" 
FROM (
	SELECT DISTINCT mem12_cvn_publicacion_usuario.publicacion_id, 
		...
		FROM mem12_cvn_publicacion
			...
			WHERE YEAR(fecha)=2012 AND 
						tipo_de_produccion="Capitulo de Libro" AND
...
/*INTO OUTFILE "/tmp/capitulos_libros_departamentos_2012.csv" FIELDS TERMINATED BY '|' LINES TERMINATED BY '\n';*/
